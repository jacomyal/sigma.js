 ...
 * examples/api-candy.html code sample to know more.
 */
(function() {
  'use strict';
   ...
   * to calculate the X and Y coordinates from the `cam` or `renderer` node
  ...
  sigma.plugins.dragNodes = function(s, renderer, realCoors) {
    ...
    var _size = s.graph.nodes().length;
    if (_size > 1) {
      ...
      var nodeMouseMove = function(event) {
        ...
        var y = event.pageY - _container.offsetTop;
        ...
        for (var i = 0; i < 2; i++) {
          ...
          aux.x = n.x;
          aux.y = n.y;
          aux.renX = n[_realCoors + ':x'];
          aux.renY = n[_realCoors + ':y'];
          ref.push(aux);
        }

        _node.x = ((x - ref[0].renX) / (ref[1].renX - ref[0].renX)) *
          (ref[1].x - ref[0].x) + ref[0].x;
        _node.y = ((y - ref[0].renY) / (ref[1].renY - ref[0].renY)) *
